...
class FacultyShowInModalForm extends Component {
  ...
  componentDidMount() {
    fetch(`/api/faculty/${encodeURI(this.state.DataView.department + "/" + this.state.DataView.id)}`).then(res => {
                ...
                <div class='facultyBoxModalMain'>
                  <div class='facultyBoxModalClose'>
                    <div class='BoxCloseButton'>
                      <Link prefetch href={`/juet/faculty/#${data.id}${encodeURI(data.department.toString().trim())}`}>
                        <a style={{ color: "white" }} onClick={this.HandleModalCloseClick}>
                          &times;
                        </a>
                      </Link>
                    </div>
                  ...
                  <div class='facultyBoxUpper'>
                    <div class='facultyBoxProfilePic'>
                      ...
                      <div class='displayDesignation'>{data.designation}</div>
                    </div>
                    <div class='facultyBoxProfileDetails'>
                      <div class='detailsContainer'>
                        <ul>
                          <li>
                            <label>Education:</label> {data.education}
                          </li>
                          <li>
                  ...
                  <div class='facultyBoxLower'>
                    <p>{Buffer.from(data.biography).toString("utf8")}</p>
                  </div>
                </div>
              </div>
            );
